...
### 2026-06-11 追記: 宮城県DB-Search候補のrobotsサンプル

宮城県内候補から5件を選び、`robots.txt` のみ確認した。ページ本文や検索結果は巡回していない。

| 自治体 | robots.txt | 結果 |
|---|---|---|
| 仙台市 | `https://net.city.sendai.miyagi.dbsr.jp/robots.txt` | `User-agent: *` は `Disallow: /`。`/$`, `/index.php$`, `/index.php/$` のみ許可。 |
| 岩沼市 | `https://www.city.iwanuma.miyagi.dbsr.jp/robots.txt` | 同上 |
| 角田市 | `https://www.city.kakuda.miyagi.dbsr.jp/robots.txt` | 同上 |
| 大河原町 | `https://www.town.ogawara.miyagi.dbsr.jp/robots.txt` | 同上 |
| 山元町 | `https://www.town.yamamoto.miyagi.dbsr.jp/robots.txt` | 同上 |

判定: 少なくとも今回確認した宮城5件では、自治体ごとの差ではなくDB-Search製品側の共通設定に近い。Phase 3でDB-Searchを扱う場合、会議録本文や検索結果を通常クロールする設計は避け、許可された入口確認・公式PDF/HTML・明示許諾・手動投入などの経路を分ける必要がある。
...
米子市は `https://www.city.yonago.lg.jp/2924.htm` から `https://www.kensakusystem.jp/yonago-s/index.html` へリンクしている。`https://www.kensakusystem.jp/yonago/` も200応答だが、公式導線上の入口ではないため補助扱いにする。
...
- 米子市議会 会議録検索（公式リンク先）: https://www.kensakusystem.jp/yonago-s/index.html
- 米子市議会 会議録検索（旧/別入口として200確認）: https://www.kensakusystem.jp/yonago/
