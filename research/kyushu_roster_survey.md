...
追記: 2026-06-15 沖縄県、福岡・宮崎・鹿児島のrobots/写真パス最終確認  
対象外: 熊本県は実装済み
...
ただし、6県は完全にバラバラではなく、次の3系統に分けられる。沖縄県を追加しても、この3系統内に収まる。

| 系統 | 県 | 見立て |
| --- | --- | --- |
| 一覧 + 個別プロフィール型 | 福岡・宮崎・鹿児島・沖縄 | 新規の `static_member_profile` 系アダプタで共通化できる可能性が高い。個別ページの許可リストパースが前提。 |
...
実装順としては、横展開効果が大きい **福岡・宮崎・鹿児島・沖縄の「一覧 + 個別プロフィール型」** から着手するのがよい。既存の熊本アダプタを拡張して無理に吸収するより、`members_cms_table.py` は熊本型として残し、別に `members_static_profile.py` を作る方が破綻しにくい。
...
| 沖縄 | https://www.pref.okinawa.jp/kensei/gikai/1017039/1029763/1029769.html | 静的HTML一覧 + 個別プロフィール | 氏名、ふりがな、会派、選挙区、写真URL、公式プロフィールURL、委員会、役職。議席番号・議員居室もあるが当面不要。 | robots.txt は404で明示禁止なし。写真は `/_res/projects/default_project/_page_/...` 配下で、写真ディレクトリDisallowは確認できず。 | そのままは不可。福岡・宮崎・鹿児島と同じ新アダプタ候補。 |

## robots / 写真パス最終確認（static_member_profile 着手候補）
...
- 公式入口: https://www.pref.okinawa.jp/kensei/gikai/index.html
- 議員名一覧入口: https://www.pref.okinawa.jp/kensei/gikai/1017039/1029763/index.html
- 五十音順: https://www.pref.okinawa.jp/kensei/gikai/1017039/1029763/1029769.html
- 選挙区別: https://www.pref.okinawa.jp/kensei/gikai/1017039/1029763/1029796.html
- 会派別: https://www.pref.okinawa.jp/kensei/gikai/1017039/1029763/1029799.html
- 委員会別: https://www.pref.okinawa.jp/kensei/gikai/1017039/1029763/1029800.html
- 個別プロフィール例: https://www.pref.okinawa.jp/kensei/gikai/1017039/1029763/1029780/1029819.html
- 様式:
  - 議員名一覧ページから、五十音順・選挙区別・会派別・委員会別に分岐する。
  - 五十音順ページは、氏名リンク + 会派 + 選挙区 + 所属委員会が並ぶ静的HTML一覧。
  - 各氏名リンクから議員単位の個別プロフィールページへ遷移する。
  - 個別ページには顔写真、氏名、住所、電話番号、選挙区、会派、委員会、議席番号、議員居室、関連情報リンクがある。
- 取得可能:
  - 一覧: 氏名、ふりがな、会派、選挙区、委員会、公式プロフィールURL。
  - 個別: 写真URL、選挙区、会派、委員会、役職、公式サイト等の関連リンク。
  - 当選回数は今回確認したページ群では見当たらないため `null` が妥当。
- 注意:
  - 個別ページには住所・電話番号が掲載されている。取得対象外として明示的にスキップする。
  - 「議員居室」「議席番号」は公開情報だが、現行membersスキーマの主目的からは外れるため当面取得しない。
- robots:
  - `https://www.pref.okinawa.jp/robots.txt` は404。明示禁止なし扱いとし、低頻度取得に留める。
  - 写真は `/_res/projects/default_project/_page_/...` 配下。写真ディレクトリの明示的なDisallowは確認できず。
- アダプタ見立て:
  - 福岡・宮崎・鹿児島と同じ `static_member_profile` 型候補。
  - ただし当選回数が無い、住所・電話が強く混在する、公式外部サイトリンクがある、という差分がある。
...
- 沖縄は一覧に会派・選挙区・委員会があり、個別ページで写真を取れる。当選回数は未掲載。
...
- 九州・沖縄7県のうち、連絡先等の個人情報が個別ページに混在する県が多い。実装時は既存方針どおり、ページ全体から自動抽出するのではなく、許可リスト方式で必要項目だけを読む。
- 写真URLは、今回の確認範囲では7県とも名簿・プロフィール配下に明示的なrobots禁止は確認されなかった。ただし、実装直前に各県robots.txtを再確認する。
